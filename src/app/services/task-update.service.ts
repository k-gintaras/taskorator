...
export class TaskUpdateService {
  constructor(private taskService: TaskService, private f: FeedbackService) {}
  ...
  split(task: Task, taskOne: Task, taskTwo: Task) {
    ...
    if (name1 !== name2 && name1 !== name3 && name2 !== name3) {
      this.taskService.createTasks([taskOne, taskTwo]).then((tasks: Task[]) => {
        this.f.log('Tasks split');
      });
    } else {
